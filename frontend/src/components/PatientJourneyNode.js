...
const getNodeColor = (type) => {
  const colors = {
    ...
    custom: 'border-gray-400 bg-gradient-to-br from-gray-50 to-white',
  };
  return colors[type] || 'border-gray-400 bg-white';
};
...
const PatientJourneyNode = ({ data, selected }) => {
  const nodeClass = `custom-node ${getNodeColor(data.stage_type)} ${selected ? 'selected' : ''}`;
